...
def on_large_root(request):
    print(request.parent_archive_path)
    print(request.file_count, request.dir_count)
    print(request.sample_entries)
    return "continue"
...
result = extract_task(
    ...
    ExtractOptions(
        ...
        prompt_on_large_extracted_root=True,
        extracted_root_fast_track_file_threshold=64,
        extracted_root_fast_track_dir_threshold=12,
        extracted_root_preview_limit=12,
        extracted_root_decision_handler=on_large_root,
    ),
)
...
当调用方传入 `extracted_root_decision_handler` 时，ArcWeaver 会在检测到“大目录内容”后回调调用方，由调用方自己决定是否继续递归探测。`request.sample_entries` 的数量受 `extracted_root_preview_limit` 控制。
